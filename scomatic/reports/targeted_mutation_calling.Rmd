...
refs <- xfun::cache_rds({
  refs <-
    ...
    tidyr::separate_wider_delim(cols = pos, delim = stringr::regex(':|-'), names = c('chr', 'start', 'end')) %>%
...
muts <-
  ...
  # dplyr::filter(!nt %in% c('-', '_', 'INS', 'ins')) %>%
...
unique(sites$gene) %>%
  purrr::walk(function(i) {
    p <-
      ...
      ggh4x::facet_grid2(celltype ~ mut_pos, scales = 'free_x') +

...
tnanoseq <-
  '/lustre/scratch125/casm/team268im/lh22/synovium_nanoseq/ex_t_dndsout/dnds_nano_muts.tsv' %>%
  readr::read_tsv()

# read genes of interest
target_genes <-
  '/lustre/scratch125/casm/team268im/at31/RA_som_mut/scomatic/data/driver_genes/Sanger_TERT-v4_TE-95148282_hg19_highstringencyfilter_buccal_gene_list.tsv' %>%
  readr::read_tsv()
...
# get chain file
chain <-
  rtracklayer::import.chain('data/liftOver/hg19ToHg38.over.chain')
    ...
    purrr::map(function(id) {
      id %>% 
        purrr::map(function(ct_bam) {
          ...
          recursites %>%
            # dplyr::filter(gene == 'TNFRSF14', aachange == 'W12*') %>% 
            purrr::pmap(function(chr, pos, start20, stop20, gene, ...) {
              # testing: # recursites %>% dplyr::filter(gene == 'TNFRSF14', aachange == 'W12*', pos == mut_pos) %>% as.list() %>% list2env(envir = globalenv())
              
              # positions
              positions <- (pos - 20):(pos + 20)
              
              # get reads at each position of the window
              deepSNV::bam2R(
                ct_bam,
                chr = chr,
                # extend pileup window 20bp either side of mutated position
                ...
                mq = mapq) %>%
                tibble::as_tibble() %>%
                dplyr::mutate(chr = chr, pos = positions, mut_pos = pos, gene = gene) %>%

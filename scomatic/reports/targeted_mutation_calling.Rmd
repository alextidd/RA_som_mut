...
plot_positions <- function(reads, include_indels = T) {
  p <-
    names(reads) %>%
    purrr::set_names() %>%
    purrr::map(function(gene) {
      ...
      if (nrow(p_dat) > 0) {
        ...
        # get plot dims
        g_p_dims <-
          c(min(max(5, dplyr::n_distinct(p_dat$mut_pos) * 2 + 2), 50),
            min(max(5, dplyr::n_distinct(p_dat$celltype) * 1.7 + 2), 50))
...
## Mutations types, by patient across genes

```{r muts_by_patient, fig.width = 10}
reads %>%
  dplyr::bind_rows() %>%

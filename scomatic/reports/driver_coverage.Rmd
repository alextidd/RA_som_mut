...
Genes of interest in rheumatoid arthritis (hereafter referred to as 'drivers') are defined as genes that are found to undergo significant selection (q-value < 0.2) according to the whole exome and targeted NanoSeq data. The coverage of these sites in the dataset, across samples and celltypes, will be checked.

First, we read in the dNdS results and filter by q-value.

```{r read_dnds, warning = F, message = F}
...
```{r plot_drivers, class.source = "fold-hide", warning = F, message = F, fig.width = 10}
drivers %>%
...
We use `samtools depth` to get the coverage of each site in the drivers, both across each sample and within each cell type in each sample.

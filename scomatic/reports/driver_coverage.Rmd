...
# function: plot all
plot_all <- function(p_df, p_exon_coords, info) {
  ggplot() +
    geom_rect(
      data = p_exon_coords,
    ...
    labs(title = paste(info, '(all)'))
}

# function: plot exons 
plot_exons <- function(p_df, title) {
  p_df %>%
    ...
    facet_grid(. ~ exon, space = 'free_x', scales = 'free_x') +
    theme_classic() +
    theme(axis.text.x = element_blank(),
          axis.ticks.x = element_blank(),
          panel.spacing = unit(.05, "lines"),
          panel.border = element_rect(fill = NA)) +
    labs(title = paste(info, '(exons)'))
}
  ...
  cov %>% 
    ...
    purrr::map(function(df_i) {
      ...
      list(by_sample = list(lvl = 'drivers_samtools_depth', title_suffix = ''),
           by_celltype = list(lvl = 'drivers_samtools_depth', title_suffix = '_by_celltype')) %>%
        purrr::map(function(x){
          ...
          # facet grid for celltypes
          if (title_suffix == '_by_celltype') {
            p_all <- p_all + facet_grid(~ celltype)
            p_exons <- p_exons + facet_grid(~ celltype)
          }
          
          # assign globally
          p[[info]][[paste0('all', title_suffix)]] <<- p_all
          p[[info]][[paste0('exons', title_suffix)]] <<- p_exons
...
for (info in names(p)) {
  ...
  knit_plot <- c(
    ...
    paste0('```{r ', info, ', class.source = "fold-hide", warning = F, message = F, ',
           'fig.dim = c(20, ', 3 * length(p[[info]]), ')}'),
    paste0('do.call("grid.arrange", c(p[["', info, '"]], ncol = 1))')
    '```',
    '')

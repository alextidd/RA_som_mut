...
load(file.path(lh22_dir, "Figures/Figure_2/LCM_NanoSeq_Overlap/GOI",
               "pileup_muts.rda"))
# muts <- list(exome = exome_muts,
#              LCM = LCM_muts,
#              targeted = nano_muts,
#              pileup = pileup_muts)

# get NF1 mutations, convert to granges
...
# get transcripts in the range
trs <-
  geneModelFromTxdb(TxDb.Hsapiens.UCSC.hg19.knownGene,
                    org.Hs.eg.db, gr = gr_ls$NF1)

entrezID <- get("NF1", org.Hs.egSYMBOL2EG)
theTrack <- geneTrack(entrezID, TxDb.Hsapiens.UCSC.hg19.knownGene)[[1]]
...
features <- c(range(trs[[1]]$dat), range(trs[[2]]$dat))
names(features) <- c(trs[[1]]$name, trs[[2]]$name)
...
lolliplot(gr_ls$NF1, features,
          ranges = GRanges("chr17", IRanges(29421945, 29704695, names = "NF1")), 
          type = "circle")
dev.off()
...
seqlevels(broads) <- gsub("chr", "", seqlevels(broads))

# get gene annotations from broads
feats <- broads[seqnames(broads) %in% seqlevels(gr)]
findOverlaps(gr, broads)

feats <-
  GenomicRanges::GRanges("12", IRanges::IRanges(start = 6702320, end = 6702330),
                         strand = "+", gene = "NF1")

trackViewer::lolliplot(SNP.gr = head(gr, 1), features = feats)
```

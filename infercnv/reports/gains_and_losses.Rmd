...
cnv_regions <- xfun::cache_rds({
  cnv_regions <- 
    ...
    dplyr::mutate(
      ...
      cell_type = cell_group_name %>% gsub('\\..*', '', .))
...
p_dat %>%
  ggplot(aes(x = pos, fill = ploidy, group = ploidy)) +
  geom_density(data = p_dat %>% dplyr::filter(direction == 'dupli'),
               aes(y = ..density..), 
               position = position_stack(), 
               colour = NA) +
  geom_density(data = p_dat %>% dplyr::filter(direction == 'loss'),
               aes(y = -..density..), 
               position = position_stack(reverse = T), 
               colour = NA) +
  ggh4x::facet_grid2(chr ~ cell_type, scales = 'free', independent = 'x') +
  theme_bw() +
...
de_res <- xfun::cache_rds({
  ...
  de_res <-
    glmGamPoi::test_de(
      fit, 
      contrast = 
        cond(chr7_gain = 1, cell_type = 'Stromal cell') -
        cond(chr7_gain = 0, cell_type = 'Stromal cell')
    )
...
Next, we plot the top differentially expressed genes onto the plot of locally amplified / lost regions within chr7 to see where they fall.

```{r plot_des_on_regions}

...
## Assigning whole chromosome gains

Based on the observed distribution of `proportion_dupli_chr7` values and comparing this with the `inferCNV` heatmaps, whole chromosome gains are defined as `proportion_dupli_chr7 > 0.8`. chr7 WT is defined as `proportion_dupli_chr7 < 0.5`.
  ...
  paste0('chr', 1:22) %>%
    purrr::walk(function(chr) {
      ...
      sce@colData[, paste0(chr, '_gain')] <<-
        ifelse(chr_dupli > 0.8, 1, ifelse(chr_dupli < 0.5, 0, NA))
      chr_loss <-
        sce@colData[, paste0("proportion_loss_", chr)]
      sce@colData[, paste0(chr, '_loss')] <<-
        ifelse(chr_loss > 0.8, 1, ifelse(chr_loss < 0.5, 0, NA))
    })
...
p_dat %>%
  ggplot(aes(x = events, fill = cell_group, label = label)) +
  geom_bar() +
  geom_text(aes(y = n), hjust = 0, vjust = -0.1, angle = 45, size = 2) +
  scale_x_discrete(guide = guide_axis(angle = -90)) +
  ylim(c(0, max(p_dat$n) * 1.1)) +
  theme_classic()
p_dat %>%
  ggplot(aes(x = forcats::fct_infreq(events), fill = sample, label = label)) +
  geom_bar() +
  geom_text(aes(y = n), hjust = 0, vjust = -0.1, angle = 45, size = 2) +
  scale_x_discrete(guide = guide_axis(angle = -90)) +
  ylim(c(0, max(p_dat$n) * 1.1)) +

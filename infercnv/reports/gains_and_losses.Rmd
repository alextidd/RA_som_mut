...
First, we load the CITE-seq object. We convert it to a SingleCellExperiment object. 
...
Next, we add the CNV metadata from the inferCNV run to the object.
  ...
  purrr::map(function(direction) {
    list(
      matrix = 
        ...
        as.matrix(),
      annot = p_dat %>% dplyr::select(sample, cell_type)
    )
  })
...
pseudobulk <- xfun::cache_rds({
  ...
  reduced_sce <-
    glmGamPoi::pseudobulk_sce(
      sce, 
      group_by = vars(chr7_gain, cell_type)
    )
